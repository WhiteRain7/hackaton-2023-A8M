...
class BaseSevice:
    ...
    def generate_table_slide(
        self, prs: Presentation, data: CreatePresentation, title: str, content_func
    ):
    ...
    async def generate_description_slide(
        ...
        image_stream = await self.generate_image(
            f"description image high quality {prompt}"
        )
    ...
    def generate_investing_rounds_slide(
        self, prs: Presentation, data: CreatePresentation
    ):
    ...
    def generate_roadmap_slide(self, prs: Presentation, data: CreatePresentation):
        ...
        for i, step in enumerate(steps):
            ax.barh(
                i,
                (step.end_date - step.start_date).days,
                left=step.start_date,
                height=0.5,
                align="center",
            )
    ...
    def generate_business_units_slide(
        self, prs: Presentation, data: CreatePresentation
    ):
    ...
    def parse_int(self, v):
        ...
        if match:
            parsed_int = int(match.group())
            return parsed_int * mul
        return 0
    ...
    def generate_tracktion_slide(self, prs: Presentation, data: CreatePresentation):
        ...
        revenue = [self.parse_int(item.revenue) for item in tracktion]
        capitalization = [self.parse_int(item.capitalization) for item in tracktion]
    ...
    def generate_finance_slide(self, prs: Presentation, data: CreatePresentation):
        ...
        description.text = (
            ...
            + f"Число клиентов: {data.clients_count}\n"
            + f"Churn Rate: {data.churn_rate}%\n"
            + f"APRU = {int(data.revenue) / int(data.clients_count)}руб\n"
            + f"LT = {1 / (int(data.churn_rate) / 100)} лет\n"
            + f"LTV = {(int(data.revenue) // int(data.clients_count)) * (1 // (int(data.churn_rate) / 100))}\n"
        )
    ...
    def generate_prompt(self, value: str) -> str:
        return (
            call_api(
                f"create an 5 base key words list based on next text separated with comma on english:{value}"
            )
            .split(":")[-1]
            .replace("\n", ", ")
            .replace(" ", ", ")
